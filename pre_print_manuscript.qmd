...
When considering only the new studies published since Hatzigeorgiadis et al. [-@hatzigeorgiadisSelfTalkSportsPerformance2011] the frequentist multilevel model estimate (`r round(tidy_rma.mv_model$estimate[1],2)` [95% confidence interval: `r round(tidy_rma.mv_model$conf.low[1],2)`, `r round(tidy_rma.mv_model$conf.high[1],2)`]) was very similar to both their prior estimate, and also the posterior Bayesian estimate from our updated model (see section above). For the conditional PET-PEESE estimator however the PET estimate was not statistically significant at the $\alpha=0.10$ level and this adjusted estimate was compatible with a range of estimates from negative effects of similar magnitude to the previosly reported positive effect in Hatzigeorgiadis et al. [-@hatzigeorgiadisSelfTalkSportsPerformance2011], through to null and trivial positive(`r round(tidy_pet_model$estimate[1],2)` [95% confidence interval: `r round(tidy_pet_model$conf.low[1],2)`, `r round(tidy_pet_model$conf.high[1],2)`]) suggesting that publication bias/small study effects may be present in this literature. These estimates can be seen in the contour enhanced funnel plot in @fig-contour-plot.
...
When considering only the post-2011 studies and utilising default prior distributions, robust Bayesian meta-analysis found strong evidence against the effect, $BF_{10}=$`r round(summary(null_robma_model)$components$inclusion_BF[1],3)` with mean model-averaged estimate of `r round(summary(null_robma_model)$estimates$Mean[1],2)` [95% quantile interval: `r round(summary(null_robma_model)$estimates[1,3],2)`, `r round(summary(null_robma_model)$estimates[1,3],2)`, strong evidence in favor of the heterogeneity, $BF_{rf}$ = `r round(summary(null_robma_model)$components$inclusion_BF[2],3)` with mean model-averaged estimate $\tau=$`r round(summary(null_robma_model)$estimates$Mean[2],2)` [95% quantile interval: `r round(summary(null_robma_model)$estimates[2,3],2)`, `r round(summary(null_robma_model)$estimates[2,3],2)`. Robust Bayesian meta-analysis found strong evidence in favor of the publication bias, BF_pb = `r round(summary(null_robma_model)$components$inclusion_BF[3],3)`. When including the prior from Hatzigeorgiadis et al. [-@hatzigeorgiadisSelfTalkSportsPerformance2011] results were similar with robust Bayesian meta-analysis finding strong evidence against the effect, $BF_{10}=$`r round(summary(prior_robma_model)$components$inclusion_BF[1],3)` with mean model-averaged estimate of `r round(summary(prior_robma_model)$estimates$Mean[1],2)` [95% quantile interval: `r round(summary(prior_robma_model)$estimates[1,3],2)`, `r round(summary(prior_robma_model)$estimates[1,3],2)`, strong evidence in favor of the heterogeneity, $BF_{rf}$ = `r round(summary(prior_robma_model)$components$inclusion_BF[2],3)` with mean model-averaged estimate $\tau=$`r round(summary(prior_robma_model)$estimates$Mean[2],2)` [95% quantile interval: `r round(summary(prior_robma_model)$estimates[2,3],2)`, `r round(summary(prior_robma_model)$estimates[2,3],2)`. Robust Bayesian meta-analysis found strong evidence in favor of the publication bias, BF_pb = `r round(summary(prior_robma_model)$components$inclusion_BF[3],3)`.
